...
class EnrollmentRequestRepository {
  ...
  static const String _defaultParentPassword = '123456';
  ...
  Future<EnrollmentApprovalResult> approveAndCreateAccount({
    ...
    final parentUserId = await _createOrUpdateParentUser(request, now);
    final parentId = await _createOrUpdateParentProfile(
      request: request,
    ...
    return EnrollmentApprovalResult(
      ...
      parentId: parentId,
      studentId: studentId,
      defaultPassword: _defaultParentPassword,
    );
  }
  ...
  Future<String> _createOrUpdateParentUser(
    EnrollmentRequestModel request,
    String now,
  ) async {
    final documentId = _safeDocumentId(
      request.phone.trim().isNotEmpty
          ? request.phone.trim()
          : (request.email ?? 'parent_$now'),
    );
    ...
    final response = await http.patch(
      ...
      body: jsonEncode({
        'fields': {
          ...
          'passwordHash': {'stringValue': _defaultParentPassword},
  ...
  Future<String> _createOrUpdateParentProfile({
    required EnrollmentRequestModel request,
    required String parentUserId,
    required String now,
  }) async {
    final parentId = _safeDocumentId(parentUserId);
    ...
    final response = await http.patch(
      ...
      body: jsonEncode({
        'fields': {
          ...
          'relationshipToStudent': {'stringValue': 'Phụ huynh'},
          'note': {'stringValue': ''},
          'createdAt': {'timestampValue': now},
        },
      }),
