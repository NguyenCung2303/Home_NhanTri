...
class _AdminEnrollmentRequestsScreenState extends State<AdminEnrollmentRequestsScreen> {
  ...
  void _reload() {
    setState(() => _future = _repository.getAll(status: _filterStatus));
  }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _InfoLine(icon: Icons.person, text: 'Phụ huynh: ${item.parentName}'),
          ...
          if (canProcess) ...[
            ...
            Wrap(
              ...
              children: [
                ...
                ElevatedButton.icon(onPressed: onApprove, icon: const Icon(Icons.check_circle), label: const Text('Duyệt')),
                TextButton.icon(onPressed: onReject, icon: const Icon(Icons.close), label: const Text('Từ chối')),
              ],
            ),
          ],
          if (item.isApproved) ...[
